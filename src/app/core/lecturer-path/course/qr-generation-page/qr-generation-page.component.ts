  ...
  async generateQr() {
    ...
    if (this.course_id && this.validity_period > 0) {
      this.qrCodeService.generatePasscode(this.course_id, this.validity_period, this.date).subscribe(async response => {
        ...
        const link = `https://backend-files-for-attendance.vercel.app/student-dashboard/scanQR?passcode=${encodeURIComponent(qrData.passcode)}&course_id=${encodeURIComponent(qrData.course_id)}&date=${encodeURIComponent(qrData.date)}`;
